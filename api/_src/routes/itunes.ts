// iTunes Search API proxy (plan §D.1), serverless port of server/src/routes/itunes.ts —
// identical logic, duplicated rather than cross-imported (see api/index.ts's comment on why
// cross-workspace imports don't survive Vercel's function bundling). No CORS, ~20 req/min rate
// limit — always proxy, cache, and debounce on the client. Also proxies the 30s preview audio
// so decodeAudioData works without a CORS-tainted buffer.
