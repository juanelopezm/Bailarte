...
function buildPrompt(analysis: VisionAnalysis, stats: DanceStats): string {
  ...
  const movementQuality = stats.smoothness > 0.55 ? 'fluidos y continuos' : 'angulares y entrecortados';

  return [
    `Crea una pintura digital abstracta de estilo ${analysis.artStyleReferences.join(', ')} que interprete un baile en vivo.`,
    `El baile: ${analysis.danceStyle}, en la tradición ${analysis.culture}. Estado de ánimo: ${analysis.mood}.`,
    `Se sintió como: "${analysis.perceivedExperience}".`,
    `Movimiento: gestos ${movementQuality}, ${durationSec} segundos de baile, ${stats.jumps} saltos, ${stats.spins} giros,`,
    `${wristTravel} metros recorridos por las muñecas, ${syncPct}% sincronizado con el ritmo a ${stats.bpm || '—'} BPM.`,
    `Enfatiza estas cualidades de movimiento: ${analysis.movementKeywords.join(', ')}.`,
    ...
    'Sin personas, sin caras, sin texto, sin marca de agua. Calidad de museo.',
  ].join(' ');
}
